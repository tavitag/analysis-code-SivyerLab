...
RGC_Atlas_coordinates = pd.read_csv("Macaque_per_RGC_coordinates.txt", index_col=0)
...
RGC_Atlas = pd.read_csv("Macaque_per_RGC_expression.txt")
...
Clust_A_mod_IDs = [identifier.replace("-1", ".1") for identifier in Clust_A_cells]
Clust_B_mod_IDs = [identifier.replace("-1", ".1") for identifier in Clust_B_cells]
...
#Clust_A_filt = RGC_Atlas[["GENE"] + list(Clust_A_uni_IDs)]
#Clust_B_filt = RGC_Atlas[["GENE"] + list(Clust_B_cells)]

Clust_A_filt = RGC_Atlas[["GENE"] + list(Clust_A_mod_IDs)]
Clust_B_filt = RGC_Atlas[["GENE"] + list(Clust_B_mod_IDs)]
...
# Get the top 20 most differentially expressed genes
top_diff_expr = diff_expr.head(20)
...
output_file_path = f"{Clust_A}v{Clust_B}_top20.txt"
